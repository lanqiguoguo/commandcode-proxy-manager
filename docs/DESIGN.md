...
- 免费模型（model 以 `:free`/`-free` 结尾，大小写不敏感，如 `LongCat-2.0:free`）请求的
  候选集放宽"额度受限"排除：额度受限（quotaLimited）的 Key 仍可服务免费模型（真实 CC
  实测：weekly 100% 硬停下免费模型返回 200，不消耗付费额度）。该放宽只作用于候选集
  与池等待出口，三策略选择、429 退避/同 Key 重试/切换/冷却语义与付费请求完全一致；
  付费请求对额度受限 Key 仍完全不可见（T6）。
...
- 额度受限（`quotaLimitedUntil` 有效）的 Key 在受限窗口内仍可服务免费模型
  （`:free`/`-free` 后缀，见 §7.1）；此类 free 成功不会清除 Key 的额度受限状态
  与 `lastErrorKind="quota"` 徽标，受限解除后按正常成功清理（T6）。
